...
function consoleDataMapper(data) {
    if (data.length > 0) {
        return <div>{data.map((item, index) => <p key={index}>{item}</p>)}</div>
    }
}
function AutomationConsole({isOpen, setIsOpen, consoleData}) {
    return (
        <div className='w-full border border-black w-[30%]'>
            <button onClick={() => setIsOpen(false)}><CrossSvg/></button>
            
            {consoleDataMapper(consoleData)}
        </div>
    );
}
